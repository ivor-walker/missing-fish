...
maximiser <- function(data, posteriors) {
  ...
  k <- ncol(posteriors) # Number of groups (age classes)
  N <- nrow(data)       # Number of data points

  # Initialize updated parameter estimates
  muHat <- c()
  sigmaHat <- c()
  lambdaHat <- c()

  # For each group, update the parameters
  for (j in 1:k) {
    # Extract posterior probabilities for group j
    P_ij <- posteriors[, j]

    # Update mu_k
    muHat <- c(muHat, sum(P_ij * data$Length) / sum(P_ij))

    # Update sigma_k
    sigmaHat <- c(sigmaHat, sqrt(sum(P_ij * (data$Length - muHat[j])^2) / sum(P_ij)))

    # Update lambda_k
    lambdaHat <- c(lambdaHat, sum(P_ij) / N)
  }
